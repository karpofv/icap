...
?>
	<div class="row-fluid" style="background: #FFFFFF;margin: 10px auto 10px auto;width: 80%;">
    	<div class="span12">
    		<!-- BEGIN BASIC PORTLET-->
    		<div class="widget blue">
    			<div style="color: #A50000;width: 100%;padding: 8px 0px 8px 7px; height: auto;font-weight: bold;overflow: hidden;font-size: 11pt;border: 1px solid #DDDDDD;background: #FAFAFA;;">
    				<h4><i class="icon-reorder"></i>  Configuraci&oacute;n del Perfil <b> (<?php echo $nombre_pefil; ?>)</b></h4>    
        			<a style="float:right;margin-right: 15px;margin-top: -10px;" onclick="var msg = confirm('Esta seguro que desea eliminar este Perfil?');
        if (msg) { $.ajax({ type: 'POST', url: 'controller.php',
        data: 'eliminar=<?=$_POST[idperfil]?>&idsubmenu=<?php echo $idsubmenu; ?>',
        success: function(html) { $('#page-content').html(html); $('#veroferta').click(); },
...
<?php
				$indicemenu=1;
				$resultx =paraTodos::arrayConsulta('*','m_menu_emp_menj','');
...
<?php
                        	...
                        	if ($_POST[idperfil]<>'') {
								...
								foreach ($consulta_sub_menu_perfiles as $resultado2){
                                	...
                                	if  ($consultar==1){$psubMenu1= "<i style='color: #36be00;' class='glyph-icon icon-check'></i>"; $accC=0;}
                                	if  ($consultar==0){$psubMenu1= "<i style='color: #FF0000;' class='glyph-icon icon-clock-os'></i>"; $accC=1;}
                                	$insertar=$resultado2["I"];
                                	if  ($insertar==1){$psubMenu2= "<i style='color: #36be00;' class='glyph-icon icon-check'></i>"; $accI=0;}
                                	if  ($insertar==0){$psubMenu2= "<i style='color: #FF0000;' class='glyph-icon icon-clock-os'></i>"; $accI=1;}
                                	$modificar=$resultado2["U"];
                                	if  ($modificar==1){$psubMenu3= "<i style='color: #36be00;' class='glyph-icon icon-check'></i>"; $accM=0;}
                                	if  ($modificar==0){$psubMenu3= "<i style='color: #FF0000;' class='glyph-icon icon-clock-os'></i>"; $accM=1;}
                                	$eliminar=$resultado2["D"];
                                	if  ($eliminar==1){$psubMenu4= "<i style='color: #36be00;' class='glyph-icon icon-check'></i>"; $accE=0;}
                                	if  ($eliminar==0){$psubMenu4= "<i style='color: #FF0000;' class='glyph-icon icon-clock-os'></i>"; $accE=1;}
                                	$imprimir=$resultado2["P"];
                                	if  ($imprimir==1){$psubMenu5= "<i style='color: #36be00;' class='glyph-icon icon-check'></i>"; $accImp=0;}
                                	if  ($imprimir==0){$psubMenu5= "<i style='color: #FF0000;' class='glyph-icon icon-clock-os'></i>"; $accImp=1;}
                        		} 
                        	}
                        	if ($entre=='') {
...
?>
                            	<a title="<?php echo 'Seleccionar '.$menu.'('.$submenu1.')'; ?>" onclick="$.ajax({ type: 'POST', url: 'controller.php',
                            data: '&act=3&org=<?php echo $idMenut;?>&idsubmenupp=<?php echo $idSubMenuP; ?>&accC=<?php echo $accC; ?>&permiso=1&submenu=<?php echo $submenuID; ?>&menus=<?php echo $idmenus; ?>&idperfil=<?php echo $_POST[idperfil]; ?>',
...
?>
                        		<a title="<?php echo 'Seleccionar '.$menu.'('.$submenu1.')'; ?>" onclick="$.ajax({ type: 'POST', url: 'controller.php',
                            data: '&act=3&org=<?php echo $idMenut;?>&idsubmenupp=<?php echo $idSubMenuP; ?>&accI=<?php echo $accI; ?>&permiso=1&submenu=<?php echo $submenuID; ?>&menus=<?php echo $idmenus; ?>&idperfil=<?php echo $_POST[idperfil]; ?>',
...
<?php
                        ...
                        <td id="modificartd<?php echo $idSubMenuP; ?>" class="CeldaRecojeDatos" height="20">
                            ...
                            if ($permiso_accion['S']==1 AND $permiso_accion['I']==1 AND $permiso_accion['U']==1 AND $permiso_accion['D']==1) { 
?>
                        		<a title="<?php echo 'Seleccionar '.$menu.'('.$submenu1.')'; ?>" onclick="$.ajax({ type: 'POST', url: 'controller.php', data: '&act=3&org=<?php echo $idMenut;?>&idsubmenupp=<?php echo $idSubMenuP; ?>&accM=<?php echo $accM; ?>&permiso=1&submenu=<?php echo $submenuID; ?>&menus=<?php echo $idmenus; ?>&idperfil=<?php echo $_POST[idperfil]; ?>', success: function(html) { $('#modificartd<?php echo $idSubMenuP; ?>').html(html); $('#thebutton').click(); },
                            error: function(xhr,msg,excep) { alert('Error Status ' + xhr.status + ': ' + msg + '/ ' + excep); }
                            });  return false;" href="javascript: void(0);">
                            		<?php echo $psubMenu3; ?>
                            	</a>
...
?>
                        		<a title="<?php echo 'Seleccionar '.$menu.'('.$submenu1.')'; ?>" onclick="$.ajax({ type: 'POST', url: 'controller.php',
                            data: '&act=3&org=<?php echo $idMenut;?>&idsubmenupp=<?php echo $idSubMenuP; ?>&accE=<?php echo $accE; ?>&permiso=1&submenu=<?php echo $submenuID; ?>&menus=<?php echo $idmenus; ?>&idperfil=<?php echo $_POST[idperfil]; ?>',
...
?>
                        		<a title="<?php echo 'Seleccionar '.$menu.'('.$submenu1.')'; ?>" onclick="$.ajax({ type: 'POST', url: 'controller.php',
                            data: '&act=3&org=<?php echo $idMenut;?>&idsubmenupp=<?php echo $idSubMenuP; ?>&accImp=<?php echo $accImp; ?>&permiso=1&submenu=<?php echo $submenuID; ?>&menus=<?php echo $idmenus; ?>&idperfil=<?php echo $_POST[idperfil]; ?>',
