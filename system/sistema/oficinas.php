<?php
    ...
    if ($permiso_accion['S']==1 AND $permiso_accion['I']==1 AND $permiso_accion['U']==1 AND $permiso_accion['D']==1) {
        ...
        if ($editarrt=="" and $descrip!=""){
			/*Se verifica no se encuentre ya registrada la oficina de lo contrario se realiza el insert*/
			$resultx = paraTodos::arrayConsultanum("ofic_descrip", "oficinas", "ofic_descrip = '$descrip'");
			if ( $resultx > 0){
                echo '<h3 class="error">Oficina ya esta registrado</h3>';
			} else {
				$insertar = paraTodos::arrayInserte("ofic_descrip","oficinas","'$descrip'");
                if ($insertar) {
                    echo '<h3 class="message">Oficina creada</h3>';
                }else{
        ...
        if ($codigo<>"0" and $descrip<>"" and $editarrt!=""){
			...
			$modifico = paraTodos::arrayUpdate("ofic_descrip='$descrip'", "oficinas", "ofic_codigo='$codigo'");			
            if($modifico){
        ...
        if ($editarrt!=""){
			$resultsedes = paraTodos::arrayConsulta("*", "oficinas", "ofic_codigo = '$editarrt'");
            foreach ($resultsedes as $rowses){
                $codigo=$rowses["ofic_codigo"];
                $descrip=$rowses["ofic_descrip"];
			}
        }
		//------------------------------------------------------------------------------------------------------------
		/*BORRAR-----------------Se verifica si la variable $borrar!="" para proceder a eliminar el usuario*/
        if ($borrar > 0) {
			$delete = paraTodos::arrayDelete("ofic_codigo = '$borrar'","oficinas");
            if ($delete) {
                echo '<h3 class="message">Oficina eliminada</h3>';
            }
        }		
		//------------------------------------------------------------------------------------------------------------	
?>
	...
	<article class="col-sm-12 col-md-12 col-lg-6 sortable-grid ui-sortable">
		...
		<div class="jarviswidget jarviswidget-sortable" id="wid-id-2" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-custombutton="false" role="widget" style="position: relative; opacity: 1; left: 0px; top: 0px;">
			<header role="heading">
				<h2>Registro de Oficinas </h2> <span class="jarviswidget-loader" style="display: none;"><i class="fa fa-refresh fa-spin"></i></span> </header>
			<!-- widget div-->
			<div role="content">
				<!-- widget edit box -->
...
<?php
	...
	if ($permiso_accion['S']==1) {
		$resultc = paraTodos::arrayConsulta("*", "oficinas", "ofic_codigo<>0")
?>
		<article class="col-sm-12 col-md-12 col-lg-6 sortable-grid ui-sortable">
			<!-- Widget ID (each widget will need unique ID)-->
			<div class="jarviswidget jarviswidget-color-darken jarviswidget-sortable" id="wid-id-0" data-widget-editbutton="false" role="widget">
				<header role="heading">
					<h2>Oficinas Registradas</h2> <span class="jarviswidget-loader"><i class="fa fa-refresh fa-spin"></i></span> 
				</header>
				<!-- widget div-->
				<div role="content">
					<!-- widget content -->
...
?>
									<tr style="border-bottom: 1px solid #EEEEEE;">
										<td style="padding: 15px 7px 15px 7px;"> <b> <?php printf("%s",$rowc[ofic_descrip]);?></b> </td>
										<td>
<?php
...
?>
        			 	...
        			 	$.ajax({
        			 		type: 'POST',
        			 		url: 'controller.php',
        					data: { editar: '<?php echo $rowc[ofic_codigo]; ?>', org: <?php echo $org; ?>},
        					success: function(html) {
        						$('#content').html(html);
        					},
        				...
        				if (msg) {
        					$.ajax({
        						type: 'POST',
        						url: 'controller.php',
        						data: { borrar: <?php echo $rowc[ofic_codigo]; ?>, org: <?php echo $org; ?>},
