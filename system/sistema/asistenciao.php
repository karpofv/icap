...
if ($permiso_accion['S']==1) {	
		$resultc = paraTodos::arrayConsulta("*", "expediente e, estatus es, expediente_func ef, funcionarios f", "e.exp_estatus=es.esta_codigo and e.exp_codigo=ef.expf_expcodigo and ef.expf_funcedula=f.fun_cedula and esta_descrip='ASISTENCIA OBLIGATORIA' and e.exp_codigo NOT IN (select asis_expcodigo from asistencia_o ao)");
?>
		<article class="col-sm-12 col-md-12 col-lg-6">
			...
			<div class="jarviswidget jarviswidget-color-darken jarviswidget-sortable" id="wid-id-0" data-widget-editbutton="false" role="widget">
				<header role="heading">
					<h2>Expedientes por aplicar asistencia obligatoria</h2>
<?php
...
<?php
    ...
    if ($permiso_accion['S']==1 AND $permiso_accion['I']==1 AND $permiso_accion['U']==1 AND $permiso_accion['D']==1) {
        ...
        if ($editarrt=="" and $motivobserv!=""){
			...
			$resultx = paraTodos::arrayConsultanum("asis_codigo", "asistencia_o", "asis_funcedula = '$cedula' and asis_expcodigo=$expcodigo");
			if ($resultx > 0){
				echo '
				...
				$insertar = paraTodos::arrayInserte("asis_fecha, asis_expcodigo, asis_funcedula, asis_observacion, asis_tipor, asis_lugarr, asis_horas, asis_supervisor, asis_observaofic, asis_pondera, asis_tipo","asistencia_o","'$fecini',$expcodigo,$cedula,'$motivobserv','$tiporeentre','$lugarreent','$canthoras','$superv','$observofic','$pondera','OBLIGATORIA'");
                if ($insertar) {
		...
		/*MOSTRAR---------------------Se verifica si la variable $editarr!="" para proceder a Mostrar los datos guardados del usuario*/
        if ($editarrt!="" and $asiscod==""){
			$resultsedes = paraTodos::arrayConsulta("*", "asistencia_o a, funcionarios f", "a.asis_funcedula=f.fun_cedula and asis_codigo=$editarrt");
            foreach ($resultsedes as $row){
        ...
        if ($borrar<>"") {
			$delete = paraTodos::arrayDelete("asis_codigo = $borrar","asistencia_o");
            if ($delete) {
				echo '
					<div class="alert alert-block alert-success">
						<a class="close" data-dismiss="alert" href="#">×</a>
