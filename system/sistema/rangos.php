<?php
    ...
    if ($permiso_accion['S']==1 AND $permiso_accion['I']==1 AND $permiso_accion['U']==1 AND $permiso_accion['D']==1) {
        ...
        if ($editarrt=="" and $descrip!=""){
			/*Se verifica no se encuentre ya registrado el rango de lo contrario se realiza el insert*/
			$resultx = paraTodos::arrayConsultanum("rang_descrip", "rangos", "rang_descrip = '$descrip'");
			if ( $resultx > 0){
                echo '<h3 class="error">rango ya esta registrado</h3>';
			} else {
				$insertar = paraTodos::arrayInserte("rang_descrip","rangos","'$descrip'");
                if ($insertar) {
                    echo '<h3 class="message">rango creado</h3>';
                }else{
        ...
        if ($codigo<>"0" and $descrip<>"" and $editarrt!=""){
			...
			$modifico = paraTodos::arrayUpdate("rang_descrip='$descrip'", "rangos", "rang_codigo='$codigo'");			
            if($modifico){
        ...
        if ($editarrt!=""){
			$resultsedes = paraTodos::arrayConsulta("*", "rangos", "rang_codigo = '$editarrt'");
            foreach ($resultsedes as $rowses){
                $codigo=$rowses["rang_codigo"];
                $descrip=$rowses["rang_descrip"];
			}
        }
		//------------------------------------------------------------------------------------------------------------
		/*BORRAR-----------------Se verifica si la variable $borrar!="" para proceder a eliminar el usuario*/
        if ($borrar > 0) {
			$delete = paraTodos::arrayDelete("rang_codigo = '$borrar'","rangos");
            if ($delete) {
                echo '<h3 class="message">rango eliminado</h3>';
            }
        }		
		//------------------------------------------------------------------------------------------------------------	
?>
	...
	<article class="col-sm-12 col-md-12 col-lg-6">
		...
		<div class="jarviswidget jarviswidget-sortable" id="wid-id-2" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-custombutton="false" role="widget" style="position: relative; opacity: 1; left: 0px; top: 0px;">
			<header role="heading">
				<h2>Registro de Rangos </h2> <span class="jarviswidget-loader" style="display: none;"><i class="fa fa-refresh fa-spin"></i></span> </header>
			<!-- widget div-->
			<div role="content">
				<!-- widget edit box -->
...
?>
									<tr style="border-bottom: 1px solid #EEEEEE;">
										<td style="padding: 15px 7px 15px 7px;"> <b> <?php printf("%s",$rowc[rang_descrip]);?></b> </td>
										<td>
<?php
...
?>
        				...
        				if (msg) {
        					$.ajax({
        						type: 'POST',
        						url: 'controller.php',
        						data: { borrar: <?php echo $rowc[rang_codigo]; ?>, org: <?php echo $org; ?>},
